...
Leaflet renders optional interactive maps using MapTiler's OpenStreetMap-based tiles. Maps visualize coordinates already authorized and returned by FloodNet; they never replace the official ward, operational jurisdiction or Express authorization rules. If the map provider is unavailable, coordinate entry and direct OpenStreetMap links remain usable.
...
- interactive exact-location selection and report map preview;
...
- incident map for reports that contain exact coordinates;
...
- interactive operational and public centre maps using only centres returned by the current authorized API query.
...
For a production-style local run, build the Vite frontend before starting Express:
...
| `VITE_MAPTILER_API_KEY` | Public, read-only MapTiler browser key used for interactive maps; restrict it to FloodNet's allowed HTTP origins. |
| `DEMO_PASSWORD` | Optional private password for local demonstration accounts. |

Only variables beginning with `VITE_` are exposed to browser code. The MapTiler browser key is intentionally public and must be origin-restricted; never put AWS credentials, database passwords, service tokens or JWT secrets in a `VITE_` variable.
